...
$(".selectTipoHabitacion").change(function(){
  ...
  $.ajax({
    ...
    url:urlPrincipal+"ajax/subcategorias.ajax.php",
    method: "POST",
    data: datos,
    cache: false,
    contentType: false,
        ...
        $.ajax({
          ...
          success:function(respuesta){
              ...
              if(respuesta.length != 0){
                ...
                colDerReservas(respuesta[0]["tipo"],respuesta[0]["estilo"],respuesta[0]["id_h"]);
...
function colDerReservas(tipo, estilo, id){

   $(".tituloReserva").val("Habitación "+tipo+" "+estilo);

   $(".pagarReserva").attr("infoHabitacion", tipo+" "+estilo);
   
   $(".pagarReserva").attr("idHabitacion", id);
